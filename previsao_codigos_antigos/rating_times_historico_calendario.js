...
async function atualizarLambdaHistoricoDaData(client, DATA_ALVO, ratingsCalculadosMap) {
    ...
    for (const jogo of jogosDia) {
        ...
        if (!ratingCasa || !ratingFora) {
            ...
            // --- INÍCIO DA FILA DE BACKFILL ---
            // Salva na tabela os times que não bateram a cota de 10 jogos
            const timesFaltando = [];
            
            if (!ratingCasa) {
                timesFaltando.push({ 
                    id: jogo.id_time_casa, 
                    fs_id: jogo.flashscore_id_time_casa 
                });
            }
            if (!ratingFora) {
                timesFaltando.push({ 
                    id: jogo.id_time_fora, 
                    fs_id: jogo.flashscore_id_time_fora 
                });
            }

            for (const t of timesFaltando) {
                await client.query(`
                    INSERT INTO fila_times_sem_historico 
                    (id_time, flashscore_id_time, liga_alvo, data_identificacao)
                    VALUES ($1, $2, $3, $4)
                    ON CONFLICT (id_time) DO NOTHING
                `, [t.id, t.fs_id, jogo.flashscore_slug_liga, DATA_ALVO]);
            }
            // --- FIM DA FILA DE BACKFILL ---
...
async function processarData(client, DATA_ALVO, indiceAtual, totalDatas) {
  ...
  WITH JogosDoDia AS (
    SELECT DISTINCT
      ...
      c.flashscore_id_casa AS flashscore_id_time_casa,
      c.flashscore_id_fora AS flashscore_id_time_fora
    FROM calendario c
    WHERE c.data_jogo = $2
      AND c.flashscore_id_casa IS NOT NULL
      AND c.flashscore_id_fora IS NOT NULL
      AND c.flashscore_slug_liga IS NOT NULL
  ),
  ...
  JogosHistoricos AS (
    ...
    JOIN jogos j
      ON j.flashscore_slug_liga = tdm.liga_alvo
     AND j.estatisticas_coletadas = 1
     AND j.data_jogo < $2
     AND j.data_jogo >= $3
     AND (
...
// =========================================================
// 📚 BUSCAR TODAS AS DATAS DA TABELA CALENDARIO
