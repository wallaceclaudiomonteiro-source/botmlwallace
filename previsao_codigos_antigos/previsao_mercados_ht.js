...
async function buscarEscanteiosHistoricos(
    ...
    try {
        ...
        if (!linhas || linhas.length === 0) {
            console.log(
                `[CANTOS DEBUG] Nenhuma linha de comportamento encontrada para ${flashscoreIdCasa} x ${flashscoreIdFora} | GE.`
            );
            return null;
        }
        ...
        function extrairBloco(timeId, tipo) {
            const linha = linhas.find(item =>
                String(item.flashscore_id_time) === String(timeId) &&
                item.tipo_analise === tipo
            );
        ...
        const blocos = {
            casa_producao: casaProducao,
            casa_sofrimento: casaSofrimento,
            fora_producao: foraProducao,
            fora_sofrimento: foraSofrimento
        };
        ...
        for (const [nome, bloco] of Object.entries(blocos)) {
            if (
                ...
                console.log(
                    `[CANTOS DEBUG] GE | Bloco ${nome} insuficiente (${bloco ? bloco.jogos.length : 0}/${MIN_JOGOS}).`
                );
        ...
        function calcularMedianaBloco(
            ...
            console.log(
                `[CANTOS DEBUG] GE | ${nomeBloco} | Mando=${bloco.casa_fora} | Válidos=${valoresEscanteios.length}/${bloco.jogos.length} | Mediana=${mediana !== null ? mediana.toFixed(2) : 'N/A'}`
            );
        ...
        if (
            ...
            console.log(
                `[CANTOS DEBUG] GE | Um ou mais blocos falharam no filtro mínimo.`
            );
        ...
        console.log(
            `[CANTOS SUCESSO] GE | Casa ${mediana_casa_produzidos.toFixed(2)} x ${mediana_fora_concedidos.toFixed(2)} -> λ ${lambda_casa_hist.toFixed(2)}`
        );

        console.log(
            `[CANTOS SUCESSO] GE | Fora ${mediana_fora_produzidos.toFixed(2)} x ${mediana_casa_concedidos.toFixed(2)} -> λ ${lambda_fora_hist.toFixed(2)}`
        );
...
// ==========================================
// PROCESSAMENTO PELO CALENDÁRIO
// HT = 1T + 2T
// ==========================================

async function rodarAnalisesPorData(dataAlvo) {
    const dataLimpa = String(dataAlvo).trim().replace(/[\r\n"']/g, '');
    console.log(`Iniciando população na tabela analise_jogos_ht baseada no CALENDÁRIO para a data: ${dataLimpa}...`);
    try {
        ...
        for (const jogo of jogos.rows) {
            ...
            for (const periodo of PERIODOS) {
                ...
                const ataqueCasa =
                    await buscarEstatisticasMatchup(
                        jogo.flashscore_id_casa,
                        dataLimpa,
                        'ATAQUE_DEFESA_TIME'
                    );
