...
function ehGol(evento) {
    ...
    if (
        desc.includes('anulado') || desc.includes('cancelado') ||
        desc.includes('pênalti perdido') || desc.includes('penalti perdido') ||
        desc.includes('pênalti defendido') || desc.includes('penalti defendido') ||
...
function compilarLinhasNetRating(historicoEstats, timeInfo, forcaAdv, dataJogo, idCompeticao) {
    ...
    for (const periodoConfig of periodos) {
        for (const itemTipo of tiposAnalise) {
            const ehContra = itemTipo.tipo === 'ATAQUE_DEFESA_CONTRA';
            ...
            const linha = {
                ...
                gol_marc_0_15: 0, gol_marc_16_30: 0, gol_marc_31_45: 0, gol_marc_46_60: 0, gol_marc_61_75: 0, gol_marc_76_90: 0,
                gol_sofr_0_15: 0, gol_sofr_16_30: 0, gol_sofr_31_45: 0, gol_sofr_46_60: 0, gol_sofr_61_75: 0, gol_sofr_76_90: 0,
            ...
            historicoEstats.forEach((histItem, idx) => {
                ...
                let golsTimePeriodo = [];
                let golsContraPeriodo = [];

                if (periodoConfig.nome === 'GE') {
                    golsTimePeriodo = Array.isArray(histItem.gols_time) ? histItem.gols_time : [];
                    golsContraPeriodo = Array.isArray(histItem.gols_contra) ? histItem.gols_contra : [];
                } else if (periodoConfig.nome === '1T') {
                    golsTimePeriodo = Array.isArray(histItem.gols_time_1T) ? histItem.gols_time_1T : [];
                    golsContraPeriodo = Array.isArray(histItem.gols_contra_1T) ? histItem.gols_contra_1T : [];
                } else if (periodoConfig.nome === '2T') {
                    golsTimePeriodo = Array.isArray(histItem.gols_time_2T) ? histItem.gols_time_2T : [];
                    golsContraPeriodo = Array.isArray(histItem.gols_contra_2T) ? histItem.gols_contra_2T : [];
                }

                const golsMarcadosPeriodo = ehContra ? golsContraPeriodo : golsTimePeriodo;
                const golsSofridosPeriodo = ehContra ? golsTimePeriodo : golsContraPeriodo;
...
async function buscarJogosPorData(dataJogo) {
    try {
        const query = `SELECT * FROM calendario WHERE data_jogo = $1`;
...
function inicializarEstruturaJogo(jogo) {
    return {
        ...
        timeCasa: {
            ...
            flashscore_id: jogo.flashscore_id_casa,
            nome: jogo.nome_time_casa,
            dados: [], stats_ewma: {}, forca: {}, historico_jogos_semelhantes: [], historico_estatisticas: []
        },
        timeFora: {
            id: jogo.id_time_fora,
            flashscore_id: jogo.flashscore_id_fora,
...
async function definirForcaTimes(baseDeDadosEmMemoria) {
    ...
    for (const jogo of baseDeDadosEmMemoria) {
        ...
        try {
            const queryComp = `
                SELECT casa_forca_limite_fraco, casa_forca_limite_forte, fora_forca_limite_fraco, fora_forca_limite_forte
                FROM competicoes WHERE id = $1
            `;
            ...
            // CORREÇÃO AQUI: Removida a data_referencia, buscando apenas o rating atual
            const queryRatingCasa = `
                ...
                LIMIT 1
            `;
            const resCasa = await client.query(queryRatingCasa, [flashCasa]);
            const ratingCasa = resCasa.rows[0];

            if (!ratingCasa) {
                console.log(`❌ ERRO: RATING CASA NÃO ENCONTRADO na tabela rating_times para ID = ${flashCasa}`);
            }
            ...
            const queryRatingFora = `
                ...
                LIMIT 1
            `;
            const resFora = await client.query(queryRatingFora, [flashFora]);
            const ratingFora = resFora.rows[0];

            if (!ratingFora) {
                console.log(`❌ ERRO: RATING FORA NÃO ENCONTRADO na tabela rating_times para ID = ${flashFora}`);
            }

            if (ratingCasa && limites && ratingCasa.casa_rating_ataque !== null && ratingCasa.casa_rating_defesa !== null) {
                const forcaCasa = calcularForcaPitagorica(ratingCasa.casa_rating_ataque, ratingCasa.casa_rating_defesa);
                const classeCasa = classificarForcaUnificada(forcaCasa, limites.casa_forca_limite_fraco, limites.casa_forca_limite_forte);
                
                console.log(`🧮 CÁLCULO CASA: Força Pitagórica = ${forcaCasa.toFixed(4)} => Classificado como [${classeCasa}]`);
                
                jogo.timeCasa.forca = {
                    forca: classeCasa,
                    net_rating: forcaCasa,
            ...
            if (ratingFora && limites && ratingFora.fora_rating_ataque !== null && ratingFora.fora_rating_defesa !== null) {
                const forcaFora = calcularForcaPitagorica(ratingFora.fora_rating_ataque, ratingFora.fora_rating_defesa);
                const classeFora = classificarForcaUnificada(forcaFora, limites.fora_forca_limite_fraco, limites.fora_forca_limite_forte);
                
                console.log(`🧮 CÁLCULO FORA: Força Pitagórica = ${forcaFora.toFixed(4)} => Classificado como [${classeFora}]`);
                
                jogo.timeFora.forca = {
                    forca: classeFora,
                    net_rating: forcaFora,
...
function distribuirGolsTime(timeObj, golsPorJogo) {
    ...
    for (const hist of timeObj.historico_estatisticas) {
        ...
        hist.gols_time = []; hist.gols_contra = [];
        hist.gols_time_1T = []; hist.gols_time_2T = [];
        hist.gols_contra_1T = []; hist.gols_contra_2T = [];
        ...
        for (const evento of golsJogo) {
            ...
            const gol = { minuto: minuto, tipo: golDoTime ? 'TIME' : 'CONTRA' };

            if (golDoTime) {
                hist.gols_time.push(gol);
                if (minuto <= 45) hist.gols_time_1T.push(gol);
                else hist.gols_time_2T.push(gol);
            } else {
                hist.gols_contra.push(gol);
                if (minuto <= 45) hist.gols_contra_1T.push(gol);
                else hist.gols_contra_2T.push(gol);
            }
        }
    }
}
...
async function buscarEventosGolsHistorico(timeCasa, timeFora) {
    ...
    for (const row of result.rows) {
        const evento = {
            ...
            id_time: row.time, minuto: row.minuto,
            tipo_evento: row.tipo_evento, descricao_evento: row.descricao_evento
        };
...
async function salvarAnaliseNoBanco(linhasAnalise, idTime, dataJogo) {
    ...
    try {
        await client.query(`DELETE FROM analise_comportamento_times WHERE flashscore_id_time = $1 AND data_jogo = $2`, [idTime, dataJogo]);

        const colunas = Object.keys(linhasAnalise[0]);
        const colunasSql = colunas.join(', ');
        let valoresParametros = [], valoresPlanos = [], contadorParam = 1;
        ...
        const insertQuery = `INSERT INTO analise_comportamento_times (${colunasSql}) VALUES ${valoresParametros.join(', ')}`;
...
async function main(dataAlvo) {
    ...
    try {
        ...
        if (jogos.length > 0) {
            ...
            for (const jogo of baseDeDadosEmMemoria) {
                if (jogo.timeCasa.forca.erro || jogo.timeFora.forca.erro || jogo.historico_insuficiente) continue;
                ...
                if (linhasCasa.length > 0) await salvarAnaliseNoBanco(linhasCasa, jogo.timeCasa.flashscore_id, dataAlvo);
                if (linhasFora.length > 0) await salvarAnaliseNoBanco(linhasFora, jogo.timeFora.flashscore_id, dataAlvo);
            }
...
const dataEscolhida = '2026-09-06';
main(dataEscolhida);