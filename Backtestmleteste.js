...
/*
 * ================================================================
 * CONFIGURAÇÃO DO TESTE A/B DE FALLBACK
 * ================================================================
 *
 * Edite só estas duas constantes entre uma rodada e outra do teste.
 * O resto do script é idêntico ao backtestMle.js original.
 *
 * COMPETICOES_ALVO: array de id_competicao do grupo sendo testado
 * (ver classificar_competicoes.sql pra gerar essa lista), ou null
 * pra processar todas (não recomendado durante o teste).
 *
 * OPCOES_TESTE: repassado direto pro otimizarModeloConjunto, por
 * cima dos pesos de nível 1 (que não mudam). Combinações do teste:
 *
 *   Grupo 1 (tem xG e xGOT)      -> controle único: {}
 *   Grupo 2 (tem xG, falta xGOT) -> normal:            { modoNivel2: 'renormalizado' }
 *                                    fallback padrão:   {}   (xG absorve tudo — default)
 *                                    xG+finalizações:   { modoNivel2: 'xg_finalizacoes' }
 *   Grupo 3 (falta xG e xGOT)    -> normal (sem fallback): { usarFallbackNivel3: false }
 *                                    fallback finalizações: {}  (default)
 */

const COMPETICOES_ALVO = null; // ex.: [12, 34, 56]

const OPCOES_TESTE = {};

// TESTE: descarta do histórico de treino (não só reduz peso) qualquer
// jogo que não tenha xG e xGOT completos nos dois lados. Com isso true,
// OPCOES_TESTE fica irrelevante pro modelo de gols (nunca mais existe
// nível 2/3 — só sobra nível 1), então deixe {} nessa rodada. Serve
// pra comparar direto: modelo treinado só com dado completo (mesmo
// perdendo volume) vs. modelo atual com fallback.
const SOMENTE_DADO_COMPLETO = false;
    ...
    try {
        ...
        const { rows: jogosAlvo } = await client.query(`
            ...
            WHERE data_jogo::date
              ...
              ${COMPETICOES_ALVO ? 'AND id_competicao = ANY($3)' : ''}

            ORDER BY
                data_jogo::date ASC,
                id ASC

        `, COMPETICOES_ALVO ? [dataInicio, dataFim, COMPETICOES_ALVO] : [
        ...
        for (const alvo of jogosAlvo) {
            ...
            const jogosValidos =
                jogosTreino.filter(j =>
                    ...
                    // TESTE: SOMENTE_DADO_COMPLETO descarta do treino
                    // qualquer jogo do histórico que não tenha xG e xGOT
                    // dos dois lados — nada de fallback, o jogo simplesmente
                    // não entra no fit de ataque/defesa. Se isso esvaziar o
                    // histórico de uma competição/time abaixo de 20 jogos,
                    // o alvo é pulado (perde volume, como você pediu).
                    (!SOMENTE_DADO_COMPLETO || (
                        j.xg_casa !== null &&
                        j.xgot_casa !== null &&
                        j.xg_fora !== null &&
                        j.xgot_fora !== null
            ...
            const parametros =
                await otimizarModeloConjunto(
...

                    {
                        ...
                        /*
                         * ============================
                         * TESTE A/B — ver OPCOES_TESTE no
                         * topo do arquivo. Sobrescreve só
                         * o que for passado lá (modoNivel2,
                         * usarFallbackNivel3, etc.); o resto
                         * continua nos pesos padrão acima.
                         * ============================
                         */
