...
// ==========================================
// --- FASE 1: COLETA DE JOGOS ---
// ==========================================
// ==========================================
// --- FASE 1: COLETA DE JOGOS ---
// ==========================================
async function coletaDeJogos(db, navegador) { // 👈 Alterado de 'pagina' para 'navegador'
    console.log('\n🔍 LOG FASE 1: Buscando time prioritário (coletado = 0)...');

    try {
        // 🔒 A TRANCA: Busca o time, tranca para os outros robôs e já marca como 4
        const res = await client.query(`UPDATE times SET coletado = 4 WHERE id = (SELECT id FROM times WHERE coletado = 5  ORDER BY id ASC LIMIT 1 FOR UPDATE SKIP LOCKED) RETURNING *;`);
        const time = res.rows[0];
        if (!time) {
            console.log('⚠️ LOG FASE 1: Nenhum time com status 5 encontrado. Fim da fila.');
            return false;
        }

        console.log(`✅ LOG FASE 1: Time encontrado: ${time.nome} (ID: ${time.flashscore_id})`);

        const linkJogos = `https://www.flashscore.com.br/equipe/${time.flashscore_slug}/${time.flashscore_id}/resultados/`;
        let carregouSucesso = false;

        // 🌟 ISOLAMENTO: Cria uma aba nova e limpa exclusivamente para este time
        let pagina = await navegador.newPage();

        try {
            for (let t = 1; t <= 3; t++) {
                try {
                    console.log(`📡 Tentativa ${t} de 3: Acessando página de resultados...`);
                    await pagina.goto(linkJogos, { waitUntil: 'domcontentloaded', timeout: 45000 }); 
                    await pagina.waitForSelector('.event__match', { timeout: 20000 });
                    carregouSucesso = true;
                    break; // Sai do loop se carregar com sucesso
                } catch (e) {
                    console.error(`⚠️ Falha na tentativa ${t}: ${e.message}`);
                    
                    // 🛠️ DEFESA ANTI-CRASH: Se a página desconectou (detached Frame), joga ela fora e cria outra!
                    if (e.message.includes('detached Frame') && t < 3) {
                        console.log(`♻️ Reiniciando aba corrompida...`);
                        await pagina.close().catch(() => {});
                        pagina = await navegador.newPage();
                    }
                    if (t < 3) await pausa(5, 5);
                }
            }

            if (!carregouSucesso) {
                console.error(`❌ ERRO CRÍTICO: Falha ao carregar a página de ${time.nome}. Pulando time.`);
                await client.query(`UPDATE times SET coletado = -1 WHERE id = $1`, [time.id]);
                return true;
            }

            console.log('⏳ LOG FASE 1: Carregando histórico de jogos...');
            while (true) {
                const qtdAtual = await pagina.evaluate(() => document.querySelectorAll('.event__match').length);
                if (qtdAtual >= 101) break;

                const botaoVerMais = await pagina.$('[data-testid="wcl-buttonLink"]');
                if (!botaoVerMais) break;

                const gamesCountBefore = await pagina.evaluate(() => document.querySelectorAll('.event__match').length);
                await pagina.evaluate(() => {
                    const btn = document.querySelector('[data-testid="wcl-buttonLink"]');
                    if (btn) btn.click();
                });

                try {
                    await pagina.waitForFunction((oldCount) => {
                        return document.querySelectorAll('.event__match').length > oldCount;
                    }, { timeout: 10000 }, gamesCountBefore);
                } catch { break; }
                await pausa(1, 1);
            }

            let arrayDeLinks = await pagina.evaluate(() => {
                const links = [];
                const linhas = document.querySelectorAll('.event__match, .event__match--last');
                for (const linha of linhas) {
                    const linkElement = linha.querySelector('a.eventRowLink[href*="/jogo/"]');
                    if (linkElement) {
                        let href = linkElement.getAttribute('href');
                        if (!href.startsWith('http')) href = 'https://www.flashscore.com.br' + href;
                        links.push(href);
                    }
                }
                return links;
            ...
            if (arrayDeLinks.length > 101) arrayDeLinks = arrayDeLinks.slice(0, 101);
            console.log(`🎯 LOG FASE 1: Foram coletados ${arrayDeLinks.length} links para o time ${time.nome}.`);
            
            arrayDeLinks = await filtrarLinksNovos(db, arrayDeLinks);

            if (arrayDeLinks.length === 0) {
                console.log(`✅ LOG FASE 1: O time ${time.nome} já está 100% atualizado. Nenhum jogo novo para processar.`);
                await client.query(`UPDATE times SET coletado = 10 WHERE id = $1`, [time.id]);
                return true;
            }

            // EXECUÇÃO DAS FASES EM SEQUÊNCIA
            try {
                // 🌟 CORREÇÃO DE PARÂMETRO: Passando o navegador diretamente
                const dadosCompletos = await processarJogosEmLotes(arrayDeLinks, navegador);
                await inserirJogos(db, dadosCompletos);
                await inserirEventosESumario(db, dadosCompletos);
                const pastasProntas = await prepararEstatisticas(db);
                await coletarEstatisticas(db, pastasProntas, navegador);

                console.log(`\n✅ LOG: Processamento concluído com sucesso para: ${time.nome}`);
                await client.query(`UPDATE times SET coletado = 10 WHERE id = $1`, [time.id]);
                return true;
            } catch (erroFases) {
                console.error(`❌ ERRO NAS FASES SEGUINTES (${time.nome}):`, erroFases.message);
                await client.query(`UPDATE times SET coletado = -1 WHERE id = $1`, [time.id]);
                return true;
            }
        
        } finally {
            // 🌟 FAXINA: Garante que a aba do time seja destruída no final, liberando a RAM
            if (pagina && !pagina.isClosed()) {
                await pagina.close().catch(() => {});
            }
        }
    } catch (erro) {
        console.error('❌ ERRO BD:', erro.message);
        throw erro;
    }
}
...
async function rasparDetalhesDoJogo(linkBase, navegador) {
    ...
    try {
        ...
        const dadosResumo = await paginaJogo.evaluate(() => {
            ...
            return {
                comum: { dataJogo, horaJogo, competicao, rodada, slug_liga, slug_pais, pais },
...
async function principal() {
    ...
    try {
        ...
        while (temMaisTimes) {
            ...
            temMaisTimes = await coletaDeJogos(db, navegador); 
